...
public final class ThemeColorResolver {
    ...
    /**
     * フッターヒント行の背後に敷く半透明黒の帯 (= 「ピル状」 backdrop)。
     *
     * <p>
     * フッターは {@link #TEXT_DIM} (= 暗めグレー) で描かれるため、 黄色フレームやタブと背景が
     * 被ると視認性が落ちる。 文字列の左右に padding を入れたこの帯を 1 本敷くことで、 ヒント行を
     * フォーカス フィルムとして浮かせる (= 4 原則の Contrast)。 alpha ≈ 69% で、 下のフレームを
     * ぼんやり残す。 倉庫検索 / 振り分けプレビュー など OmniChest の全フッターで再利用する
     * (= マジックナンバの分散を防ぐ + テーマ統一)。
     */
    public static final int FOOTER_BACKDROP = 0xB0000000;
